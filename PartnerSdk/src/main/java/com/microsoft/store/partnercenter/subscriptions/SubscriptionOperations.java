 ...
 */
public class SubscriptionOperations extends
		BasePartnerComponent<Tuple<String, String>> implements ISubscription {
	...
	/***
	 * A reference to the current subscription's resource usage records
	 * operations.
	 */
	 ...
	 *            The subscription id.
	 */
	public SubscriptionOperations(IPartner rootPartnerOperations,
	...
	public ISubscriptionAddOnCollection getAddOns() {
		if (this.subscriptionAddOnsOperations == null) {
			this.subscriptionAddOnsOperations = new SubscriptionAddOnCollectionOperations(
					this.getPartner(), this.customerId, this.subscriptionId);
		}
	...
	public ISubscriptionUpgradeCollection getUpgrades() {
		if (this.subscriptionUpgradeOperations == null) {
			this.subscriptionUpgradeOperations = new SubscriptionUpgradeCollectionOperations(
					this.getPartner(), this.customerId, this.subscriptionId);
		}
	...
	public ISubscriptionUsageRecordCollection getUsageRecords() {
		if (this.usageRecordsOperations == null) {
			this.usageRecordsOperations = new SubscriptionUsageRecordCollectionOperations(
					this.getPartner(), customerId, subscriptionId);
		}
	...
	public ISubscriptionUsageSummary getUsageSummary() {
		if (this.subscriptionUsageSummaryOperations == null) {
			this.subscriptionUsageSummaryOperations = new SubscriptionUsageSummaryOperations(
					this.getPartner(), customerId, subscriptionId);
		}
	...
	public IUtilizationCollection getUtilization() {
		if (this.subscriptionUtilizationOperations == null) {
			this.subscriptionUtilizationOperations = new UtilizationCollectionOperations(
					this.getPartner(), customerId, subscriptionId);
		}
	...
	public ISubscriptionProvisioningStatus getProvisioningStatus() {
		if (this.subscriptionProvisioningStatusOperations == null) {
			this.subscriptionProvisioningStatusOperations = new SubscriptionProvisioningStatusOperations(
					this.getPartner(), customerId, subscriptionId);
		}
	...
	public Subscription get() {
		{
			IPartnerServiceProxy<Subscription, Subscription> partnerServiceProxy = new PartnerServiceProxy<Subscription, Subscription>(
					new TypeReference<Subscription>() {
					}, this.getPartner(),
					MessageFormat.format(PartnerService.getInstance()
							.getConfiguration().getApis()
							.get("GetSubscription").getPath(), this
							.getContext().getItem1(), this.getContext()
							.getItem2(), Locale.US));
	...
	public Subscription patch(Subscription subscription) {
		...
		PartnerServiceProxy<Subscription, Subscription> partnerApiServiceProxy = new PartnerServiceProxy<Subscription, Subscription>(
				new TypeReference<Subscription>() {
				}, this.getPartner(), MessageFormat.format(
						PartnerService.getInstance().getConfiguration()
								.getApis().get("UpdateSubscription").getPath(),
						this.getContext().getItem1(), this.getContext()
								.getItem2(), Locale.US));
